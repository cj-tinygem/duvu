...
- **의도**: 설계자가 "왜 이렇게 만들었는가" — 시선 유도, 무게 배분, 여백의 이유
- **의미**: 사용자가 "이것이 무엇을 뜻하는가" — 행동 유도, 정보 전달, 감정 형성

각 컴포넌트에 `intent`(설계 의도)와 `meaning`(사용자 인식)이 분리되어 있다. 둘 다 참조하라.

**의도 분석 프로세스**: 모든 요소 배치 전 3단계:
1. **존재**: 이것이 없으면 안 되는가? → No면 제거
2. **정교함**: 이 크기/색상/위치가 왜 이 값이어야 하는가? → 토큰 선택의 근거
3. **관계**: 주변 요소와의 무게 균형은 맞는가? → `level` 참조

단순함은 출발점이 아니라 **도착점**이다. 모든 것에 의도를 부여한 결과로 불필요한 것이 제거되어 단순해지는 것이다. 아름다움 자체도, 의도적 불균형도 유효한 의도다.
