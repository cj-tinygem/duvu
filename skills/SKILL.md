...
모든 요소를 배치할 때, 아래 **16개 차원**에서 "왜 이 값인가?"를 판단하라. 해당 요소에 관련된 차원만 확인하면 된다.
...
**사용법**: "이 버튼은 왜 이 크기인가?" → level 5이므로 가장 크게. "이 여백은 왜 넓은가?" → restraint: high. 매번 16개를 전부 묻는 것이 아니라, **해당 결정에 관련된 차원만** 확인.

**시스템 데이터 참조**: 컴포넌트 `intent`/`meaning`/`level`, layout `restraint`/`harmonizes_with.why`, `page_patterns.flow`/`why`
